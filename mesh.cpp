...
void make_mesh()
{
	...
	fval * hvar = new fval[tot_hfacep];
	fval * vvar = new fval[tot_vfacep];
	...
	if(restart==0)
	  ...
	  mg_initialize(level);			
        ...
        /***Testing interpolation function and staggered derivatives***/ 
                    
	//interpol_cen_vface(fvar,vvar,0);        
       	//derv_vface2node_y(fvar, vvar,0);  //d/dy(vface_value) on cell center 
       	
       	//interpol_cen_hface(fvar,hvar,0);        
       	//derv_hface2node_x(fvar,hvar,0);  //d/dy(vface_value) on cell center 
	...
	mg_final(level,fvar,0);	
        ...
        solver(node, fvar, hvar, vvar, level, pbc, restart_count);
