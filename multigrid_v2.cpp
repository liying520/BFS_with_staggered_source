...
void poisson_source(fval * fvar, fval * hvar, fval *vvar, mg_grid * level, double Q)
{
	int ind;
        ...
        //Interpolaation to edge centers and evaluation of derivatives starts
        
        /*interpol_cen_hface(fvar,hvar,3);        
       	derv_hface2node_x(fvar,hvar,3);  //d/dy(vface_value) on cell corner 
       	
       	interpol_cen_vface(fvar,vvar,4);        
       	derv_vface2node_y(fvar, vvar,4);  //d/dy(vface_value) on cell center*/
                
        //Interpolaation to edge centers and evaluation of derivatives ends
        
        tdma1x(fvar,3);  //From the original collocated code
        tdma1y(fvar,4);
...
void compute_pbc(fval * fvar, pbcs& pbc, int up)	
{
	  ...
	  tdma1x(fvar,0);  //Evaluate du/dx at all points in the domain required for outflow BC
	  tdma1x(fvar,1);  //Evaluate dv/dx at all points in the domain required for outflow BC
	  ...
	  { 
	  	...
	  	{    
			...
			/*pbc.p1y[i] = 0.0;  	//Bottom boundary //Both are Neumann BCs for Kovasznay flow
	  	...
	  	for(int j=0;j<=ny;j++) //Getting the pressure gradient at the left and right faces of the domain 
	  	{
	     		...
	     		pbc.p2x[j] = (1./Re)*( fvar[ind2].uxx[0] + fvar[ind2].uyy[0] );
	     		
	     		//pbc.p2x[j] = (1./Re)*( fvar[ind2].ux[0] ); //Right boundary
	     		
	     		//pbc.p1x[j] = (1./Re)*( fvar[ind1].uxx[0] + fvar[ind1].uyy[0] );     				//Left boundary
	     		
	     		//pbc.p2x[j] = (1./Re)*fvar[ind2].ux[0] - (1./4.)*( (fvar[ind2].u[0]*fvar[ind2].u[0]) + (fvar[ind2].u[1]*fvar[ind2].u[1]) )*(1. - tanh(fvar[ind2].u[0]/delta)); //Dirichlet boundary condition from the paper by Dong et al //OBC - C type*/
	     			     			     		
	     		//Kovasznay BC 	     			     			     		
	     		/*pbc.p1x[j] = -lambda*exp(-2.*lambda*0.5);	     		
	     		pbc.p2x[j] = (1./Re)*fvar[ind2].ux[0]
	     		pbc.p2x[j] = (1./Re)*fvar[ind2].ux[0] - (1./4.)*( (fvar[ind2].u[0]*fvar[ind2].u[0]) + (fvar[ind2].u[1]*fvar[ind2].u[1]) )*(1. - tanh(fvar[ind2].u[0]/delta)); //Dirichlet boundary condition from the paper by Dong et al //OBC - C type*/ 
