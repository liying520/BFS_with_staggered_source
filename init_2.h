...
const int nx=128, ny=32, nz=20; 

const int str_x = nx+1; //stride for the collocated grid 
const int cstr_x = nx;  //stride for the face central grid in x-direction 
 
const int file_freq=1; // Frequency of writing down files for data visualization
...
const double l_x = 40.0;   //The length of the channel
const double delta_p = 2.0; //This is the difference between the outlet and inlet pressure. The flow is from left to right

const double Re = 400.0; //Reynolds number based on the size of the cavity 
...
typedef struct field          // Defining the structure for the field variables
{
	...
	double u0[NUM_VAR]; 
	double F,div,res,dp,rhs,err,dudt; 		
}fval; 
...
const int tot_hfacep = nx*(ny+1);   //Total no. of hface points 
const int tot_vfacep = (nx+1)*ny;   //Total no. of vface points 
